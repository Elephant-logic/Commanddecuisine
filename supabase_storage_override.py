...
def _bucket():
    return os.environ.get('SUPABASE_STORAGE_BUCKET', 'command-de-cuisine').strip() or 'command-de-cuisine'
...
def status(handler):
    ...
    handler.send_json({
        ...
        'urlConfigured': bool(_url()),
        'secretConfigured': bool(_key()),
        'bucket': _bucket(),
        'role': user.get('role'),
    })
...
def upload_from_json(handler, payload):
    ...
    try:
        ...
        if len(data) > 12 * 1024 * 1024:
            raise ValueError('Uploads are limited to 12 MB per file on this build')
        stamp = datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')
        result = _upload(f'Documents/{stamp}-{name}', data, mime_type)
...
def maybe_daily_backup(state, revision):
    ...
    with _LOCK:
        if _LAST_BACKUP_DATE == today:
            return None
        result = _upload(f'Backups/command-de-cuisine-{today}.json', _backup_payload(state, revision), 'application/json')
...
def backup_now(handler):
    ...
    try:
        stamp = datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')
        result = _upload(f'Backups/command-de-cuisine-{stamp}.json', _backup_payload(stored['state'], stored['revision']), 'application/json')
        handler.send_json({'ok': True, 'file': result})
    except Exception as exc:
        handler.send_json({'error': str(exc)}, 400)
