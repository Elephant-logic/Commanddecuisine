...
# Discover the existing authenticated password-change endpoint from the production server
# instead of hard-coding a route that may drift between bundled releases.
...
# Use the route nearest the change_password call.
route = routes[-1]

src = Path('self_password_change.js')
js = src.read_text(encoding='utf-8').replace('__PASSWORD_ROUTE__', route)
...
# Load after all manager/team UI patches so the button survives their DOM/script cleanup.
index = app / 'index.html'
html = index.read_text(encoding='utf-8')
tag = '<script src="/self_password_change.js?v=20260915-pw1"></script>'
...
# Self-check the final build artifacts.
...
if "'/self_password_change.js'" not in final_server:
    raise SystemExit('Self password script route missing')
if final_html.count(tag) != 1:
    raise SystemExit('Self password script tag not installed exactly once')
if '__PASSWORD_ROUTE__' in final_js or route not in final_js:
    raise SystemExit('Password API route was not wired into client script')

print(f'Self-service password change restored using {route}')
